...
export const getImageDataBuffer = async (imageData: Blob) => {
  const arrayBuffer = await blobToArrayBuffer(imageData) as ArrayBuffer;
  const imageDataArray = new Uint8Array(arrayBuffer);
  const isJPEG = checkImageDataIsJPEG(imageDataArray);

  if (isJPEG) {
    return imageDataArray;
  } else {
    console.log('Error: Image does not have JPEG file signature')
  }
}